...
export function resolveChatGptWebModelMode(
  ...
  const astra = modelId === CHATGPT_WEB_ASTRA_MODEL_ID;
  if (!astra && modelId !== CHATGPT_WEB_MODEL_ID) {
    throw new Error(`ChatGPT web model is not supported: ${modelId}`);
  }
  if (!capabilities.solAvailable) {
    throw new Error("ChatGPT Web model families are not available for this Luna-only account");
  }
  if (astra && !capabilities.proAvailable) {
    throw new Error("ChatGPT Astra modes are not available for this account");
  }
  const uiFamily = astra ? "Latest" as const : "GPT-5.6 Sol" as const;
  const effort = reasoning ?? "high";
  switch (effort) {
    case "low":
      return { modelId, effort, displayLabel: "Instant", uiEffortIndex: 0, uiFamily, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "medium":
      return { modelId, effort, displayLabel: "Medium", uiEffortIndex: 1, uiFamily, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "high":
      return { modelId, effort, displayLabel: "High", uiEffortIndex: 2, uiFamily, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "xhigh":
      if (!capabilities.proAvailable) throw new Error("ChatGPT Extra High effort is not available for this account");
      return { modelId, effort, displayLabel: "Extra High", uiEffortIndex: 3, uiFamily, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    case "max":
      if (!capabilities.proAvailable) throw new Error("ChatGPT Pro effort is not available for this account");
      return { modelId, effort, displayLabel: "Pro", uiEffortIndex: 4, uiFamily, thinkEnabled: false, localTools: capabilities.localToolsEnabled };
    default:
